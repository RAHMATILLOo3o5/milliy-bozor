...
/**
 * This is the model class for table "dayticket".
 *
 * @property int $id
 ...
 */
class Dayticket extends \yii\db\ActiveRecord
{
    /**
     * {@inheritdoc}
     */
    ...
    public function rules()
    {
        return [
            ...
            [['sale', 'price', 'status', 'order', 'created_at', 'updated_at'], 'integer'],
            [['name'], 'string', 'max' => 255],
        ];
    }
    ...
    {
        if (!empty($this->limit_uz)) {
            $option = explode('.', $this->limit_uz);
            $result = [];
            for ($i = 0; $i < count($option); $i++) {
                $result[] = '<p class="p-0 m-0"><i class="fas fa-fw fa-times mr-2 text-danger"></i> ' . $option[$i] . ' </p>';
            }
            return implode(' ', $result);
        }
    }
    ...
    /**
     * {@inheritdoc}
     * @return \backend\models\search\DayticketQuery the active query used by this AR class.
     */
    // public static function find()
    // {
    //     return new \backend\models\search\DayticketQuery(get_called_class());
    // }
}
